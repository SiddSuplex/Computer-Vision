...
def randomize_augment(image, steering_angle):
  ...
  #Returning the augmented image with its steering angle
  return image, steering_angle
...
def image_generator(img_paths, steering_angles, batch_size, is_training):
  ...
  while(True):
    ...
    batch_steering_list.append(angle)
...
x_val_gen, y_val_gen = next(image_generator(x_val, y_val, 1, 0))
...
history = model.fit_generator(image_generator(x_train, y_train, 100, 1), steps_per_epoch=300, epochs=10, validation_data = image_generator(x_val, y_val, 100, 0), validation_steps=200, verbose=1, shuffle = 1)
