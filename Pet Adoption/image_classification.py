# -*- coding: utf-8 -*-
"""
Created on Thu Jan  9 11:15:58 2020
...
#Referred https://www.geeksforgeeks.org/working-zip-files-python/
...
train_img_dir = "training_set"
test_img_dir = "test_set"
...
for animal in glob.glob(train_img_dir + "/*"):
    for img in glob.glob(animal + "/*.jpg"):
        ...
        print(os.path.basename(animal), count)

        #Storing the labels in the empty array
        train_labels.append(os.path.basename(animal))
...
for animal in glob.glob(test_img_dir + "/*"):
    for img in glob.glob(animal + "/*.jpg"):
        ...
        #Storing the labels in the empty array
        test_labels.append(os.path.basename(animal))
...
train_img_arr = np.reshape(train_img_arr, ((8005, 128, 128, 1)))
test_img_arr = np.reshape(test_img_arr, ((2023, 128, 128, 1)))

#Denoting the training and test data
train_data = train_img_arr
test_data = test_img_arr
...
assert(train_data.shape[0] == train_labels.shape[0]), "Number of training images and labels don't match"
assert(test_data.shape[0] == test_labels.shape[0]), "Number of test images and labels don't match"
...
model.fit(train_data, train_labels, epochs = 10, validation_split = 0.2, shuffle = True)
...
#Got a test accuracy score of 0.9950568462679189 (99.5%)